...
from indsl.smooth.eweight_ma import ewma
from indsl.smooth.lweight_ma import lwma
from indsl.smooth.simple_ma import sma
...
def test_sma():
    ...
    sma_calculated = sma(sma_serie, time_window=pd.Timedelta("15min"), min_periods=1).values
...
def test_ewma(adjust):
    ...
    ewma_calculated = ewma(ewma_serie, time_window=pd.Timedelta("15min"), adjust=adjust, min_periods=1).values
    return np.testing.assert_array_almost_equal(
        ewma_calculated,
        ewma_expected,
        decimal=6,
...
def test_lwma():
    ...
    lwma_calculated = lwma(lwma_serie, time_window=pd.Timedelta("15min")).values
    return np.testing.assert_array_almost_equal(
        lwma_calculated,
        lwma_expected,
        decimal=6,
