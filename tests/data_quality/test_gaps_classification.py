...
def test_single_gap_series():
    ...
    assert result.iloc[0]["classification"] in [
        "Typical",
        "Significant",
        "Abnormal",
        "Singularities",
...
def test_multiple_gaps_series():
    ...
    assert all(
        classification in ["Typical", "Significant", "Abnormal", "Singularities", "Extreme"]
        for classification in result["classification"]
    ), "Unexpected classification found."
...
def test_irregular_sampling_series():
    ...
    assert all(
        classification in ["Significant", "Abnormal", "Singularities", "Extreme"]
        for classification in result["classification"]
    ), "Unexpected classification found."
