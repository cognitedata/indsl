...
def test_shutin_calc_one_shutin_before():
    ...
    valve_ts = pd.Series([1, 1, 1, 1, 5, 5], index=date_range).astype(float)
...
def test_shutin_calc_one_shutin_after():
    ...
    # Define input ts
    date_range = pd.date_range(start=datetime(2000, 10, 1), end=datetime(2000, 10, 10), periods=6)
    valve_ts = pd.Series([5, 5, 1, 1, 1, 1], index=date_range).astype(float)
...
def test_shutin_calc_one_shutin():
    ...
    # Define input ts
    date_range = pd.date_range(start=datetime(2000, 10, 1), end=datetime(2000, 10, 10), periods=6)
    valve_ts = pd.Series([5, 5, 1, 1, 5, 5], index=date_range).astype(float)
...
def test_threshold_calculation_error():
    # Define input ts
    date_range = pd.date_range(start=datetime(2000, 10, 1), end=datetime(2000, 10, 10), periods=6)
    valve_ts = pd.Series([1, 1, 1, 1, 1, 1], index=date_range).astype(float)
