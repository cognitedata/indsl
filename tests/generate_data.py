...
def create_uniform_data(
    ...
    values = np.hstack(values)  # type: ignore[arg-type]
    return pd.DataFrame(
...
def create_non_uniform_data(
    ...
    values_stacked = np.hstack(values)
    indices = [start_date]
    for i in range(1, len(values_stacked)):
        indices.append(indices[i - 1] + time_delta * random.randint(1, 1000))
    return pd.DataFrame({"value": values_stacked}, index=indices).iloc[:, 0]
