...
def test_positive_granularity():
    with pytest.raises(UserValueError) as e:
        test_data = pd.Series([random.random() * i for i in range(24)], index=pd.date_range("2020-02-03", periods=24, freq="1H"))
        granularity = pd.Timedelta(0, "m")
        resample_to_granularity(test_data, granularity=granularity)
        assert "Timedelta must be strictly positive. The smallest possible value is '1s'" in str(e.value)
...
def test_if_upsampled(method):
    ...
    # upsampled to 30 min
    upsampled = resample(data, granularity_next=pd.Timedelta("30T"), method=method)
...
def test_if_downsampled(method):
    ...
    # upsampled to 30 min
    upsampled = resample(data, granularity_next=pd.Timedelta("3120T"), method=method)
...
def test_test_resample_distribution(method):
    ...
    # upsampled to 30 min
    upsampled = resample(data, granularity_next=pd.Timedelta("6T"), method=method)
