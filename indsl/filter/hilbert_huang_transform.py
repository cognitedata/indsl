...
def finalize_spectrum_processing(
    ...
    output_as_sparse=False,
    dense_output_size_limit=10,
):
    """
...
def hilbert_huang_transform(
    ...
    sample_rate_hz = int(1 / (np.mean(np.diff(timestamps.to_numpy()))))
    ...
    amplitude_array = np.array(amplitude).T  # transpose to get the right shape
    ...
    trend_rho = np.sum(imfs[significant_imf_index_rho:], axis=0)
    ...
    return result_rho, result_rho_old
    ...
    #     rho_old = np.sum(hht_old[:, i] * hht_old[:, i + 1]) / (np.sum(hht_old[:, i]) * np.sum(hht_old[:, i + 1]))
    #     #rho_old = np.sum(hht_old[:, i] * hht_old[:, i + 1]) / (np.sum(hht_old[:, i]) * np.sum(hht_old[:, i + 1]))
    #     # rho = np.sum(hht_old[:, i] * hht[:, i + 1]) / (np.sum(hht[:, i]) * np.sum(hht[:, i + 1]))

    #     if rho < error_tolerance:
    #         break
    #     else:
    #         significant_imf_index -= 1

    # trend = np.sum(imfs[significant_imf_index:], axis=0)
    # trend_series = pd.Series(trend, index=signal.index)

    # print("Sum of the trend:", np.sum(trend_series))
    # print("Sum of the signal:", np.sum(analytic_signals))  # type: ignore
    # print("first point of trend", trend_series[0])

    # return trend_series if return_trend else signal - trend_series  # type: ignore
